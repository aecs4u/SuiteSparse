...
# Architecture-specific optimizations for AMD Ryzen (znver2)
if ( NOT SUITESPARSE_GCC14_OPTIMIZATIONS_SET )
    if ( CMAKE_SYSTEM_PROCESSOR MATCHES "x86_64|AMD64" AND CMAKE_C_COMPILER_ID STREQUAL "GNU" )
        if ( CMAKE_C_COMPILER_VERSION VERSION_GREATER_EQUAL 14.0 )
            # GCC 14+ with znver2 optimizations
            set ( CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE} -march=znver2 -mtune=znver2 -mavx2 -mfma" )
            set ( CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE} -fexcess-precision=fast -fcx-limited-range" )
            set ( CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE} -fno-math-errno -fwrapv" )
            set ( CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE} -ftree-loop-distribute-patterns -fhoist-adjacent-loads" )

            # Apply same flags to Fortran
            set ( CMAKE_Fortran_FLAGS_RELEASE "${CMAKE_Fortran_FLAGS_RELEASE} -march=znver2 -mtune=znver2 -mavx2 -mfma" )
            set ( CMAKE_Fortran_FLAGS_RELEASE "${CMAKE_Fortran_FLAGS_RELEASE} -fexcess-precision=fast -fno-math-errno" )

            set ( SUITESPARSE_GCC14_OPTIMIZATIONS_SET TRUE CACHE INTERNAL "GCC 14 optimizations applied" )
            message ( STATUS "SuiteSparse: Using GCC 14+ znver2 optimizations for AMD Ryzen" )
        endif ( )
    endif ( )
endif ( )
